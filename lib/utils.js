...
/** Calcula diferencia entre ítems egresados e ingresados (solo faltantes, diff > 0) */
export function calcDiferencia(egresados, ingresados) {
  const eMap = {};
  parseItems(egresados).forEach(({ nombre, cantidad }) => { eMap[nombre] = cantidad; });
  const iMap = {};
  parseItems(ingresados).forEach(({ nombre, cantidad }) => { iMap[nombre] = cantidad; });

  const diffs = [];
  for (const [nombre, cantEgr] of Object.entries(eMap)) {
    const cantIng = iMap[nombre] || 0;
    const diff = cantEgr - cantIng;
