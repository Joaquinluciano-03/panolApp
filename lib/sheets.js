...
export async function deleteRow(sheetName, rowIndex) {
  ...
  // Primero necesitamos el sheetId numérico de la hoja
  ...
  const sheetRow = rowIndex + 1; // +1 porque el índice 0 en datos = fila 2 en Sheets (fila 1 es header)
  await sheets.spreadsheets.batchUpdate({
    spreadsheetId: SPREADSHEET_ID,
    requestBody: {
      requests: [{
        deleteDimension: {
          range: {
            sheetId,
            dimension: 'ROWS',
            startIndex: sheetRow,     // 0-indexed en Sheets (header=0, datos empiezan en 1)
            endIndex: sheetRow + 1,
          },
        },
      }],
