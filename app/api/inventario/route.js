...
export async function POST(request) {
  ...
  if (existing) return NextResponse.json({ error: 'Ya existe un ítem con ese nombre' }, { status: 409 });
  ...
  await supabase.from('auditoria_stock').insert({
    id: generateId(), fecha: formatDate(now), hora: formatTime(now),
    item_id: id, item_nombre: nombre, accion: 'ALTA_ITEM',
    stock_anterior: 0, stock_nuevo: total, usuario: payload.email, diferencia: total
  });
